...
export const TableCell: React.FC<Props> = ({cellName, rowId, value, onChangeData, cellIndex, rowIndex, disabled}) => {
    ...
    return <StyledTableCell data-cellindex={cellIndex} align={'center'}
                            onKeyDown={onKeyPressHandler(cellIndex)}>
        <EditableSpan rowId={rowId} placeholder={cellName} value={value} onChange={onChangeData(cellName)} disabled={disabled}/>
    </StyledTableCell>
}