...
export const EditableTableRow: React.FC<Props> = ({rowData, onChangeData, rowIndex}) => {
    const total: string = (Number(rowData.cash) + Number(rowData.bort) - Number(rowData.gas) - Number(rowData.fuel) - Number(rowData.washing) - Number(rowData.avans) - Number(rowData.spendings)).toString()
    ...
    return <StyledTableRow data-rowindex={rowIndex}>
        {
            [
                ...
                return <TableCell key={rowData._id + item.name} onChangeData={onChangeData} cellName={item.name} value={item.data} cellIndex={index} rowIndex={rowIndex} rowId={rowData._id} disabled={item.disabled || false} />
            })
        }
    </StyledTableRow>
}